...
---Select the first 10,000 records from the journal files

SELECT * FROM jrnl_mnvt
...
SELECT COUNT(jrnl_mnvt.account) AS missing_count
FROM jrnl_mnvt t2
...
SELECT COUNT(jrnl_mnvt.rep_id) AS missing_rep
FROM jrnl_mnvt t2
...
AS num_wrong_action_type FROM jrnl_mnvt 
...
SELECT COUNT(CASE WHEN Counterparty IS NULL THEN 1 ELSE 0 END) AS num_null_counteryparty,
      ...
      COUNT (CASE WHEN Jrn_Date IS NULL THEN 1 ELSE 0 END) AS num_null_jrn_date,
      COUNT (CASE WHEN Scr_Jrn_Date IS NULL THEN 1 ELSE 0 END) AS num_null_src_jrn_date,
      COUNT (CASE WHEN Account_No IS NULL THEN 1 ELSE 0 END) AS num_null_acct
FROM jrnl_mnvt WHERE case_no='case_no' AND firm_id='firm_id';
...
SELECT DISTINCT (file_name, header_group, Counterparty, Counterparty_Acct)
FROM jrnl_mnvt WHERE case_no='case_no' AND firm_id='firm_id' LIMIT 10000;
